...
public class SSAuthenticationManager {
    ...
    private lazy var updateUserProfileURL: String = {
    ...
    public func updateUserProfile(userProfileDictionary userProfileDictionary: [String: AnyObject], completionHandler: ProfileResponse) -> Void {
        let headers = [X_TOKEN_KEY: self.accessToken!]
        self.networkManager.request(.PUT, String(format: self.updateUserProfileURL, self.userId!), parameters: userProfileDictionary, encoding: .JSON, headers: headers)
            .validate()
            .responseJSON { response in
                let statusCode = response.response?.statusCode ?? ERROR_STATUS_CODE
                switch response.result {
